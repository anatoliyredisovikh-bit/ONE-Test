--[[
    ...
    Версия 10.0 – ПРИНУДИТЕЛЬНАЯ ЗАГРУЗКА doubleBuffering
    ========================================================================
]]
...
local serialization = require("serialization")   -- системный, всегда есть
...
-- =====================================================================
--  ПРИНУДИТЕЛЬНАЯ ЗАГРУЗКА doubleBuffering (до GUI)
-- =====================================================================
print("🔧 Проверяю загрузку doubleBuffering...")

-- 1. Пытаемся загрузить через require
local db, err = pcall(require, "doubleBuffering")
if not db then
    print("❌ Не удалось загрузить doubleBuffering через require. Переустанавливаю...")
    os.execute("rm -f /lib/doubleBuffering.lua")
    os.execute("wget -f http://raw.githubusercontent.com/IgorTimofeev/GUI/master/doubleBuffering.lua /lib/doubleBuffering.lua")
    db, err = pcall(require, "doubleBuffering")
end

if not db then
    print("❌ doubleBuffering не загружен. Проверьте файл /lib/doubleBuffering.lua")
    print("Нажмите любую клавишу для выхода...")
    computer.pullEvent("key_down")
    os.exit()
end

-- 2. Делаем глобальную переменную buffer (этого требует GUI)
buffer = db
_G.buffer = db   -- на всякий случай

print("✅ doubleBuffering загружен успешно, buffer установлен глобально.")

-- =====================================================================
--  ПОДКЛЮЧЕНИЕ GUI (теперь buffer уже есть)
-- =====================================================================
...
-- =====================================================================
--  СЕРВЕРНАЯ ЛОГИКА (BACKEND) – полностью сохранена
-- =====================================================================
local Server = {}
