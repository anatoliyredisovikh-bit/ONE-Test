-- ============================================================
-- PIM MARKET SERVER + АДМИН-ПАНЕЛЬ (текстовый стиль)
-- Полностью повторяет интерфейс со скриншота
-- ============================================================
...
-- ===== ГРАФИЧЕСКАЯ АДМИН-ПАНЕЛЬ (СТИЛЬ КАК НА СКРИНШОТЕ) =====
local WIDTH, HEIGHT = 80, 25
gpu.setResolution(WIDTH, HEIGHT)

-- Только чёрный фон и белый/серый текст
...
local function drawHeader(title)
    ...
    write(WIDTH - unicode.len(status) - 1, 1, status, shopPaused and 0xFFAA00 or 0x00FF88)
end
...
local function drawMenu()
    ...
    for i, item in ipairs(menuItems) do
        ...
        local label = (i == selected) and ("## " .. item.label) or ("   " .. item.label)
        write(2, y, prefix .. " " .. label, (i == selected) and 0xFFFFFF or 0x888888)
        write(40, y, item.desc, 0x555555)
        y = y + 1
    end
...
local function drawPlayers()
    ...
    for i = 1, 15 do
        ...
        if item then
            ...
            write(22, y, string.format("%.2f", item.data.balance or 0), 0x00FF88)
            write(35, y, string.format("%.2f", item.data.emaBalance or 0), 0xFFAA00)
            write(48, y, string.format("%d", item.data.transactions or 0), 0xFFFFFF)
            local status = item.data.banned and "ЗАБАНЕН" or "АКТИВЕН"
            write(58, y, status, item.data.banned and 0xFF5555 or 0x00FF88)
            write(70, y, isAdmin(item.name) and "ДА" or "НЕТ", isAdmin(item.name) and 0x4A90E2 or 0x666666)
        end
    end
...
local function drawAddItem()
    ...
    for i, f in ipairs(addItemFields) do
        ...
        write(4, y, prefix .. " " .. f.label, (i == addItemSelected) and 0xFFFFFF or 0x888888)
        write(30, y, f.value, (i == addItemSelected) and 0xFFFFFF or 0xAAAAAA)
    end
